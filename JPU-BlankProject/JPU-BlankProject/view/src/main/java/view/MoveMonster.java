							...
							for(int j=0;j<51;j++) {
								...
								if(ViewPanel2.tabObjets[i][j]!=null) {
									...
									if(ViewPanel2.tabMonsters.get(k).isGoesRight()) {
										...
										if(ViewPanel2.tabMonsters.get(k).DoesnotmovesRight(ViewPanel2.tabObjets[i][j])) {
										...
										if(ViewPanel2.tabMonsters.get(k).DoesnotmovesLeft(ViewPanel2.tabObjets[i][j])) {
											ViewPanel2.tabMonsters.get(k).setWalks(false);
											ViewPanel2.tabMonsters.get(k).setGoesLeft(false);
										}
									}else if(ViewPanel2.tabMonsters.get(k).isGoesUp()) {
										if(ViewPanel2.tabMonsters.get(k).DoesnotmovesUp(ViewPanel2.tabObjets[i][j])) {
											ViewPanel2.tabMonsters.get(k).setWalks(false);
											ViewPanel2.tabMonsters.get(k).setGoesUp(false);
										}
									}else if(ViewPanel2.tabMonsters.get(k).isGoesDown()) {
										if(ViewPanel2.tabMonsters.get(k).DoesnotmovesDown(ViewPanel2.tabObjets[i][j])) {
											ViewPanel2.tabMonsters.get(k).setWalks(false);
